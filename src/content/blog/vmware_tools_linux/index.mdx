...
2. Copier-coller le dossier compressé `VMwareTools-xxxxxxxxxx.tar.gz` sur votre Bureau (ou autres dossiers de votre machine).
3. Décompressez-le et rendez-vous à l'intérieur.

![Dossier de VMware Tools](.img/vmware_tools_folder.png "Dossier de VMware Tools")

4. Lancer un Terminal (*clic-droit -> Ouvrir un Terminal*) et lancer la commande suivante `sudo ./vmware-install.pl`.
5. Il va ensuite vous poser plusieurs questions sur les modules à installer que vous pouvez valider par <kbd>Entrée</kbd> pour qu'il prenne les valeurs par défaut.
