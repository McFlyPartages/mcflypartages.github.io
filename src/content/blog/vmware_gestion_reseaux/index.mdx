...
1. Dans `Network Adapter`, cliquez sur le bouton `LAN Segment...`,
1. Dans la nouvelle fenêtre, cliquez sur `Add`, nommez votre réseau (ici `McFlyTuto`) et cliquez sur `OK` pour le valider,
1. Sélectionnez `LAN segment` puis choisir votre réseau nouvellement créé,
1. Validez le tout.
...
Il est intéressant de créer vos VMs en mode NAT pour les connecter à internet et bénéficier des mises à jour. Ensuite, il vous suffit de créer un LAN-Segment pour simuler votre propre réseau en y ajoutant vos VMs clientes, serveurs, routeurs ou autres.import type { servicemanagement } from "googleapis/build/src/apis/servicemanagement";
