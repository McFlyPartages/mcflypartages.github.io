...
4.1 Virtualisation des serveurs

C'est le type de virtualisation le plus courant et le plus largement utilisé.



Principe : Permet d'exécuter plusieurs systèmes d'exploitation virtuels sur un seul serveur physique.
...
4.2 Virtualisation des postes de travail (VDI - Virtual Desktop Infrastructure)

Permet de centraliser la gestion des postes de travail en les hébergeant sur des serveurs distants.



Principe : Les utilisateurs accèdent à leur bureau virtuel via un client léger ou un navigateur web.
...
4.3 Virtualisation des applications

Isole les applications de l'environnement d'exploitation sous-jacent.



Principe : L'application s'exécute dans un conteneur virtuel, indépendamment du système d'exploitation hôte.
...
Abstrait les ressources de stockage physiques pour créer des pools de stockage virtualisés.



Principe : Regroupe plusieurs dispositifs de stockage physiques en un seul dispositif de stockage virtuel géré de manière centralisée.
...
4.5 Virtualisation des réseaux (NFV - Network Function Virtualization)

Sépare les fonctions réseau du matériel réseau dédié.



Principe : Les fonctions réseau (routage, pare-feu, équilibrage de charge, etc.) sont implémentées en logiciel et peuvent s'exécuter sur du matériel standard.
...
4.6 Conteneurisation

Forme légère de virtualisation au niveau du système d'exploitation.



Principe : Encapsule une application et ses dépendances dans un "conteneur" qui peut s'exécuter sur n'importe quel système compatible.
...
4.7 Virtualisation de la mémoire

Permet de gérer la mémoire de manière plus flexible et efficace.



Principe : Abstrait la mémoire physique pour créer des pools de mémoire virtualisée qui peuvent être alloués dynamiquement.
...
4.8 Virtualisation du GPU

Permet de partager les ressources GPU entre plusieurs machines virtuelles ou conteneurs.



Principe : Divise les ressources d'un GPU physique en plusieurs GPU virtuels.
