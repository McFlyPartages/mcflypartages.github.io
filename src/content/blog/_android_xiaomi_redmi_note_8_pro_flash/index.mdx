---
draft: true
dossier: _android_xiaomi_redmi_note_8_pro_flash
...
2. Vérifiez que le déverrouillage OEM est désactivé (doit être grisé)
3. Vérifiez que le statut du déverrouillage Mi est bien `Déverrouillé`
4. Activez le débogage USB
   ![Activation du débogage USB sur MIUI](./img/miui_activation_debug_usb.gif)
5. Branchez votre smartphone à votre PC et validez le popup d'autorisation du débogage USB.
...
2. Selectionnez votre roms.zip laisser les deux cases decochées,
3. Faite glisser pour confirmer le flash en bas de l'ecran.
   Une fois le flash terminé, revenez au menu principale
4. Cliquez sur Formatter, Formatter les données, tapez `yes`,
5. Cliquez sur Redémarrer le systeme (reboot systeme)
